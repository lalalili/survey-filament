...
class SurveyInvitationEmailStatsWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        if (! class_exists(\Lalalili\EmailCampaign\Models\EmailDelivery::class)) {
            return [
                Stat::make('邀請信統計', '—')
        ...
        $counts = \Lalalili\EmailCampaign\Models\EmailDelivery::query()
            ->whereHas('campaign', fn ($q) => $q->where('survey_id', $this->record->id))
            ->where('status', \Lalalili\EmailCampaign\Enums\EmailDeliveryStatus::Sent)
